 ...
 * 
 * Run from ojs-client/: node generate-pdfs.js
 */

import mysql from 'mysql2/promise';
import https from 'https';

// Skip SSL certificate verification for the UDSM server
...
function fetchPage(url) {
  return new Promise((resolve, reject) => {
    const req = https.get(url, { timeout: 15000, agent }, (res) => {
...
async function main() {
  ...
  for (const article of localArticles) {
    if (!article.title) {
      console.log(`  Skip galley ${article.galley_id}: no title`);
      failed++;
      continue;
    }
    ...
    if (!liveIssue) {
      console.log(`  No live issue for galley ${article.galley_id}: "${article.title?.substring(0, 50)}..."`);
      failed++;
      continue;
    }

    if (!liveArticleCache[liveIssue.liveId]) {
      console.log(`Fetching live issue Vol.${liveIssue.vol} No.${liveIssue.num} (${liveIssue.year}) [ID:${liveIssue.liveId}]...`);
      try {
        ...
        for (const link of liveLinks) {
          ...
          try {
            ...
            console.log(`    ${pdfUrl ? '✓' : '✗'} Article ${link.id}`);
          } catch (err) {
            console.log(`    ✗ Article ${link.id}: ${err.message}`);
    ...
    if (bestMatch && bestScore >= 0.5 && bestMatch.pdfUrl) {
      ...
      updated++;
      console.log(`  ✓ galley ${article.galley_id} -> ${bestMatch.pdfUrl}`);
      ...
      updated++;
      console.log(`  ~ galley ${article.galley_id} -> ${viewUrl} (no PDF)`);
    } else {
      console.log(`  ✗ No match: "${article.title?.substring(0, 60)}..." (score: ${bestScore.toFixed(2)})`);
      failed++;
    }
  }
